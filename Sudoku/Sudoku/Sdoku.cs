...
namespace Sudoku
{
    class Sudoku
    {
        ...
        public class SolutionStepList  //제네릭 클래스 사용
        {
            ...
            private bool Search(SolutionStep ss)
            {
                for (int i = 0; i < L.Count; i++)
                {
                    ...
                    if (ss.x == present.x && ss.y == present.y && ss.num == present.num)
                        return true;
                }
        ...
        public void Draw(Graphics G, float angle)
        {
            ...
            //RenderMessageBox(G, "14 풀이가능한 횟수", "아무키나 눌러 계속진행", realw / 22, realw / 36, w, h, realx, realy, realw);

            //RenderMessageBox(G, "문제가 확인되지않음!", "아무키나 눌러 계속 진행", realw / 22, realw / 36, w, h, realx, realy, realw);

        }

        //프로그램 동작에 관련된 여러가지 출력 메시지 관리 코드
        ...
        //전체 퍼즐 체크
        public bool CheckGame()  //제네릭 리스트 사용
        {
        ...
        private List<int> GenerateRandomList() //리스트 제네릭 사용 예외처리
        {
            Random R = new Random();
